...
Then, we define `get_input_args` function within the file `get_input_args.py`(explanation of file [here][cla]). This function retrieves three Command Line Arugments from user as input from the user running the program from a terminal window. This function returns the collection of these command line arguments from the function call as the variable `in_arg`.
```
